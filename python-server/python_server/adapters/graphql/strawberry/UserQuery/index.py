...
@strawberry.type
class UserQuery:
    @strawberry.field(name="getUserById", description="Get a user by id")
    async def get_user_by_id(self, info: Info[GraphQLContext], id_: int) -> StrawberryUser | None:
        use_case: GetUserByIdUseCase = info.context.container.resolve(GetUserByIdUseCase)
        user: User | None = await use_case(id_)
        return StrawberryUser.from_domain(user) if user else None

    @strawberry.field(name="getUsers", description="Get all users from database")
    async def get_users(self, info: Info[GraphQLContext]) -> list[StrawberryUser]:
        ...
        return [
            StrawberryUser.from_domain(user)
            for user in users
        ]
