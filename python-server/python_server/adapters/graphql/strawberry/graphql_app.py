...
app_container: punq.Container = create_container(
...
class DIGraphQL(GraphQL):
    def __init__(
        self, schema: strawberry.Schema, container: punq.Container, **kwargs: Any
    ) -> None:
        super().__init__(schema=schema, **kwargs)
        self.container = container

    async def get_context(  # type: ignore[override]
        self, request: Request | WebSocket, response: Any
    ) -> GraphQLContext:
