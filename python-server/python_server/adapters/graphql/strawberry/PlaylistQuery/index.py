...
class PlaylistQuery:
    @strawberry.field(
        name="getPlaylistsByUserId", description="Get all playlists by a user id"
    )  # type: ignore[misc]
    ...
    async def get_playlist_by_song_id(
        self, info: Info[GraphQLContext], song_id: int
    ) -> list[StrawberryPlaylist]:
        use_case: GetPlaylistsBySongIdUseCase = info.context.container.resolve(
            GetPlaylistsBySongIdUseCase
        )
    ...
    @strawberry.field(
        name="getPlaylistById", description="Get a playlist by id"
    )  # type: ignore[misc]
    async def get_playlist_by_id(
        self, info: Info[GraphQLContext], id_: int
    ) -> StrawberryPlaylist | None:
        use_case: GetPlaylistByIdUseCase = info.context.container.resolve(
            GetPlaylistByIdUseCase
        )
